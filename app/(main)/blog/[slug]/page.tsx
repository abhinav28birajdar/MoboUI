...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen pb-24">
      {/* Header */}
      <div className="pt-24 pb-12 border-b border-zinc-200 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/30">
        <div className="mx-auto max-w-3xl px-6 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-zinc-900 dark:text-white mb-6">{post.title}</h1>
          <p className="text-xl text-zinc-600 dark:text-zinc-400 mb-8">{post.excerpt}</p>
          
          <div className="flex items-center justify-center gap-4">
            <div className="w-12 h-12 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center font-bold text-zinc-900 dark:text-white text-lg">
              {post.author.name[0]}
            </div>
            ...
            <div className="text-left">
              <p className="text-sm text-zinc-700 dark:text-zinc-300">{formatDate(post.publishedAt)}</p>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">{post.readingTime} min read</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-3xl px-6 py-12">
        ...
        <div className="mt-16 pt-8 border-t border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
          <Link href="/blog" className="text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:text-white transition-colors flex items-center gap-2">
            ← Back to Blog
          </Link>
        </div>
      </div>
