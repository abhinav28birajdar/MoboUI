...
export default function PlaygroundPage() {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden">
        ...
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex flex-col gap-2">
          {['expo', 'react-native', 'flutter'].map(f => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${fw === f ? 'bg-zinc-200 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:text-white hover:bg-zinc-50 dark:bg-zinc-900/50'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {components.map(c => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm rounded-lg transition-colors ${selectedComponent === c.slug ? 'bg-accent/10 text-accent font-medium' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-200 hover:bg-zinc-50 dark:bg-zinc-900'}`}
            >
              {c.name}
            </button>
          ))}
      ...
      {/* Panel 2: Editor */}
      <div className="flex-1 flex flex-col h-full border-r border-zinc-200 dark:border-zinc-800 bg-[#1E1E1E]">
        <div className="h-10 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex items-center px-4">
          <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">{fw === 'flutter' ? 'main.dart' : 'App.tsx'}</span>
        </div>
      ...
      {/* Panel 3: Preview */}
      <div className="w-[400px] h-full bg-white dark:bg-zinc-950 flex flex-col hidden lg:flex">
        <div className="h-10 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex justify-between items-center px-4">
           <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Preview</span>
           <select 
             className="bg-transparent text-sm text-zinc-600 dark:text-zinc-400 outline-none"
             value={activeDevice}
        ...
        <div className="flex-1 flex items-center justify-center p-4 bg-zinc-50 dark:bg-zinc-900/30 overflow-y-auto">
          {fw === 'flutter' ? (
