      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-wrap gap-2 mb-12">
          {tags.map(tag => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-full transition-colors ${filter === tag ? 'bg-zinc-800 text-white' : 'border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900'}`}
            >
              {tag}
            </button>
          ))}
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-24 bg-zinc-900/30 rounded-2xl border border-zinc-800 border-dashed">
            <p className="text-zinc-500 mb-2">No posts found</p>
