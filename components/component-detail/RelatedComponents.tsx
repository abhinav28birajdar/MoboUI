    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {related.map(comp => (
        <Link key={comp.slug} href={`/components/${comp.slug}`}>
          <Card className="h-full flex flex-col group cursor-pointer overflow-hidden border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/50 bg-zinc-950">
            <CardContent className="p-5 flex flex-col flex-1">
               <h4 className="font-medium text-white mb-2 group-hover:text-accent transition-colors">{comp.name}</h4>
               <p className="text-xs text-zinc-500 line-clamp-2 mb-4">{comp.description}</p>
