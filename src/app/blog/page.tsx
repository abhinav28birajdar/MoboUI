          ...
          {tags.map((tag) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${
                selectedTag === tag
                  ? 'bg-[#FFCA03] text-black border-[#FFCA03]'
                  : 'bg-[#111113]/40 text-[#A1A1AA] border-[#27272A]/60 hover:text-white hover:border-[#FFCA03]/40'
              }`}
            >
              {tag}
            </button>
            ...
            {featuredPost && (
              <div className="group relative">
                <Link href={`/blog/${featuredPost.slug}`} className="block">
                  <div className="bg-[#111113]/40 border border-[#27272A]/50 rounded-2xl overflow-hidden flex flex-col lg:flex-row h-full hover:border-[#FFCA03]/30 transition-all duration-300">
                    ...
                    <div className="flex-1 p-10 lg:p-16 flex flex-col justify-center space-y-6">
                      <div className="flex gap-2">
                        <span className="text-[#FFCA03] text-[10px] font-black uppercase tracking-widest">
                      ...
                      <div className="pt-6 border-t border-[#27272A]/40 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded-full bg-primary/20 border border-[#FFCA03]/30 flex items-center justify-center text-[10px] font-black text-primary">
                            M
                          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8">
                {listPosts.map((post) => (
                  <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
                    <div className="flex flex-col h-full bg-[#111113]/40 border border-[#27272A]/50 rounded-2xl overflow-hidden hover:border-[#FFCA03]/30 transition-all duration-300">
                      ...
                      <div className="p-8 flex flex-col flex-1 space-y-4">
                        <span className="text-[#FFCA03] text-[9px] font-black uppercase tracking-widest">{post.category || 'Mobile'}</span>
