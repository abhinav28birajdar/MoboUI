...
import { ArrowRight, BookOpen, Code2, Palette, Zap, Cpu, Layout, HelpCircle } from 'lucide-react';
...
export default function DocsPage() {
    return (
        <div className="container mx-auto py-20 px-6 relative overflow-hidden">
            {/* Background Glows */}
            <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-amber-500/5 blur-[150px] -z-10 rounded-full pointer-events-none" />
            ...
            >
                <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-amber-500/10 text-amber-500 text-xs font-bold uppercase tracking-widest border border-amber-500/20 shadow-[0_0_15px_rgba(132,204,22,0.2)]">
                    Documentation Hub
                </div>
                <h1 className="font-heading font-black text-6xl md:text-8xl text-white tracking-tighter uppercase  leading-none">
                    Master the <span className="text-amber-500 neon-text-glow">Toolkit.</span>
                </h1>
                ...
                {docsSections.map((section) => (
                    <motion.div key={section.title} variants={item}>
                        <Link href={section.href}>
                            <Card className="h-full border border-white/5 bg-neutral-900/50 backdrop-blur-md hover:bg-neutral-900 hover:border-amber-500/50 transition-all duration-500 group cursor-pointer overflow-hidden relative rounded-3xl p-2">
                                <CardHeader className="p-8">
                                    <div className={`mb-8 flex h-16 w-16 items-center justify-center rounded-2xl ${section.bg} border border-white/5 transition-all group-hover:scale-110 group-hover:rotate-6 group-hover:border-amber-500/30 group-hover:shadow-[0_0_20px_rgba(132,204,22,0.3)] duration-500`}>
                                        <section.icon className={`h-8 w-8 ${section.color}`} />
                                    </div>
                                    <CardTitle className="text-2xl font-heading font-black text-white mb-4 group-hover:text-amber-500 transition-colors uppercase  tracking-tight">{section.title}</CardTitle>
            ...
            >
                ...
                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
                    <div className="space-y-6 text-center lg:text-left">
                        <h2 className="text-4xl md:text-5xl font-heading font-black text-white tracking-tighter uppercase  leading-tight">
                            Ready to build? <br />
                            <span className="text-amber-500">Start with a template.</span>
                        </h2>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                        <Button size="lg" className="btn-primary h-16 px-12 rounded-2xl text-lg shadow-xl shadow-amber-500/20">
                            Explore Templates
                        </Button>
                ...
                {/* Decorative Elements */}
                <div className="absolute top-1/2 right-0 -translate-y-1/2 w-64 h-64 bg-amber-500/5 blur-[120px] rounded-full group-hover:scale-150 transition-transform duration-1000" />
            </motion.div>
        </div>
    );
}
